...
#################################################################################################################
               ...
               Separate Implementation for Question 5 only:
                   two components of latent variable and plot the reconstructed images by
                      varying the value of the latent variable.
                      In addition to plotting the output at each iteration is 
                      saved in the result folder at run time.
...
import q5_data_prepro as emnist
...
tf.flags.DEFINE_integer('epochs', 50, 'Number of steps to run trainer.')
...
tf.flags.DEFINE_string('dataset', 'emnist', 'Dataset name [emnist,emnist]')
...
train_images, train_labels, test_images, test_labels = emnist.preprocess()
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    print('This code is to show the output of 2 component latens space')
    print('Please wait to get similar out in the report, Thanks')
    sess.run(init_vars)
